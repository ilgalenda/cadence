...
"""Keep the lead-scoring website map in step with the live timebeat.app site.

The scorer ([scoring.py]) grades a visitor's page path against a map of the
timebeat.app site. This module fetches the live sitemap, pre-classifies every
real path with the built-in families, and writes an explicit {path: category}
override to ``page_map.json`` so brand-new pages are picked up automatically.

Scoring itself never touches the network — it only reads ``page_map.json`` if
present. Refreshing is done here, triggered lazily (TTL) or by an admin.
"""
import json
import re
import threading
...
from agents.lead import scoring
from paths import lead_data

SITEMAP_INDEX_URL = "https://www.timebeat.app/sitemap.xml"
PAGE_MAP_FILE = lead_data() / "page_map.json"
...
_EXPECTED_OTHER = (
    "about", "partner", "conditions-of-sale", "terms", "privacy", "cookie",
    "blank", "/test", "careers", "team", "downloads", "press", "legal",
)
...
def _collect_page_urls(client: httpx.Client) -> list[str]:
    ...
    for sm in sub_sitemaps:
        try:
            ...
            print(f"[lead.sitemap] sub-sitemap fetch failed {sm}: {e}")
    return urls
...
def maybe_refresh_page_map() -> None:
    ...
    def _run():
        try:
            ...
            print(f"[lead.sitemap] background refresh failed: {e}")
